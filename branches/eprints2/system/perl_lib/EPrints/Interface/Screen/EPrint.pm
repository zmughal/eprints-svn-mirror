...
sub render_blister
{
	...
	foreach my $stage_id ( $workflow->get_stage_ids )
	{
		...
		if( $staff_mode )
		{
			$a = $session->render_link( "eprint?eprintid=".$self->{processor}->{eprintid}."&screen=EPrint::Edit&stage=$stage_id" );
		}
		else
		{
			$a = $session->render_link( "eprint?eprintid=".$self->{processor}->{eprintid}."&screen=EPrint::Edit_staff&stage=$stage_id" );
		}
		$td->appendChild( $a );
