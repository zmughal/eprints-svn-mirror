=head1 What Additional Software does EPrints 2.0 Require? 
...
EPrints requires that the apache modules B<mod_perl> and B<mod_rewrite> be enabled.
...
=item Download the archive. Either from cpan.org, or from the tools directory on eprints.org described at the top of this chapter. Our example archive is C<FOO-5.23.tar.gz>.
