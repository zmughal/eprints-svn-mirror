...
There is an 'area' locspec which requires the 'convert' unix command. It returns an area of an image specified as C<locspec=area:topx,topy/width,height> - It is not yet supported and it's use is not recommended.
