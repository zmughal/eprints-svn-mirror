...
Copy the two functions and name them C<< eprint_render_full >> and C<< user_render_full >>, respectively.
...
eprint_render and user_render should now be altered to behave as if C<< $show_all >> had been set to false.
