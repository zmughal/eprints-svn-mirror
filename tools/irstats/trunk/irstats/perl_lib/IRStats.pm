...
=item IRStats->new( [request => REQUEST] )
...
sub new
{
	...
	{
		$self->{conf} = IRStats::Configuration->new();
	}
	$self->{views} = [];
	$self->{database} = IRStats::DatabaseInterface->new(session => $self);
...
sub get_phrase
{
	...
	return $self->get_database->phrase( $phrase );
}
