...
#	$div->appendChild( $title_div );
	...
	if (!$classified) {
		...
		$unclassified->appendChild( $plugin->{session}->make_text( "You have unclassified objects in your repository, to classify these you may want to run the bin/update_pronom_puids script. If not installed this tool is availale via http://files.eprints.org" ));
		my $warning = $plugin->{session}->render_message("warning", $unclassified);
		$message_element->appendChild($warning);
	}
