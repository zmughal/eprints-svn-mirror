=head1 What Other Software does EPrints 2.0 need? 
...
If installing from RPM you require: B<mysql-server>, B<mysql-devel> and B<mysql>. 
...
EPrints required that the apache modules B<mod_perl> and B<mod_rewrite> be enabled.
...
Remeber to change C<../apache-1.3.14/src> to wherever your apache source is relative to this directory.
...
C<XML::DOM>, C<XML::RegExp>, C<Filesys::DiskSpace>, C<URI>, C<Apache::AuthDBI>.
...
Under RedHat Linux, the B<zlib-devel> RPM should be installed before you install this module.
